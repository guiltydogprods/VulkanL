...
inline rgfx_buffer rgfx_createBuffer(const rgfx_bufferDesc* desc)
{
    ...
    bufferInfo.size = desc->capacity; // sizeof(UniformBufferData);
    bufferInfo.usage = usage;
    bufferInfo.sharingMode = VK_SHARING_MODE_EXCLUSIVE;

    VkBuffer buffer;
    VkDevice device = s_device.vkDevice;
...
//    uint32_t memoryTypeIndex = rgfx_getMemoryType(memReqs.memoryTypeBits, memoryPropertyFlags);    // , memAlloc.memoryTypeIndex);
//    rgfx_gpu_alloc_info_vk memAllocInfo = rgfx_gpu_alloc(/*scope,*/memReqs.size, memReqs.alignment, memoryTypeIndex);
	...
	s_rendererData.buffers[idx].vkBuffer = buffer;
